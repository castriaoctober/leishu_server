...
# ver 4月24日
# 4月24日更新日志：如果是模糊，搜索从boolean改为natural
# 为什么这样做？：之前在末端函数处理检索条件时，只要出现一个“模糊”，就把所有条件改为模糊
# 因此需要对于每一条做处理
# 将标准化后的关键词解析为布尔模式的 MATCH AGAINST 查询语句
...
# 为什么这样做？：之前在末端函数处理检索条件时，只要出现一个“模糊”，就把所有条件改为模糊
# 因此需要对于每一条做处理
# 根据条件列表构建 SQL WHERE 子句，支持 AND / OR 逻辑分组
...
# ver 4月14日
# 构建高级搜索的完整 SQL 查询，包括 JOIN 和条件逻辑处理
...
# ver 4月14日
# 根据 doc_id 和可选的 title_id 获取附加信息字段（标题、作者、全文摘要）
...
# ver 4月14日
# 打印筛选后的字段结果，处理分类字段并格式化为 JSON 输出
import json
def print_results(results):
    output_fields = [
        "doc_title", "category_type", "doc_specific_category",
        "doc_style", "doc_theme", "author_name",
        "author_org", "role", "title_name", "full_text"
    ]  # 需要输出的字段列表

    for row in results:
        filtered = {field: row.get(field, "") for field in output_fields}  # 提取指定字段，没有的设为空
        if "category_type" in filtered:
            val = filtered["category_type"]
            filtered["category_type"] = "类书" if val == 1 else ("引书" if val == 0 else "")  # 映射分类类型为中文
        print(json.dumps(filtered, ensure_ascii=False, indent=2, default=str))  # 格式化输出为 JSON，保持中文字符


# ver 4月14日
# 执行全字段搜索：分别查询元数据字段和正文，并整合结果
...
# **************************从前端获取用户的search_conditions，将其传给下面的 build_final_sql 即可

# # 例1：全字段检索       特征：若用户想要检索全文的模糊，效果并不好 BUT 用户给的信息多，能找到
search_conditions = [
    {"column": "author_name", "keyword": "解", "logic": "", "search_option": "精确"}
]

# # 例2：单一字段检索       特征：若用户想要检索全文的模糊，效果并不好
# search_conditions = [
#     {"column": "full_text", "keyword": "元 or 宋", "logic": "", "search_option": "精确"}
# ]

# # 例3：高级检索       特征：
# search_conditions = [
#     {"column": "author_name", "keyword": "解縉 or 崔元皙", "logic": "", "search_option": "模糊"},
...
try:
    ...
    if isinstance(sql_or_results, str):
        # print(sql_or_results) only for debug
